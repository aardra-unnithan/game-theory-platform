    ...
    <div style={s.gamePage}>
      <div style={s.topBar}>
        ...
        <div style={s.topRight}>
          ...
          <span style={s.stageBadge}>Stage {currentStage} of {totalStages}</span>
        </div>
      </div>
          ...
          <div style={s.tableSection}>
            ...
            <table style={s.table}>
              ...
              <tbody>
                {payoffTable.map((row) => (
                  <tr key={row.stage}>
                    <td style={row.isCurrent ? s.tdActive : s.td}>{row.stage}</td>
                    <td style={row.isCurrent ? s.tdActive : s.td}>{row.mover}</td>
                    <td style={row.isCurrent ? s.tdActive : s.td}>${row.studentPayoff}</td>
                    <td style={row.isCurrent ? s.tdActive : s.td}>${row.aiPayoff}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div style={s.gameRight}>
          ...
          {lastAiMove && (
            <div style={s.aiMoveBox}>
              ...
              <p style={s.aiMoveText}>
                AI chose{" "}
                <strong style={{ color: lastAiMove.move === "stop" ? "#c0392b" : "#27ae60" }}>
                  {lastAiMove.move.toUpperCase()}
                </strong>
              </p>
          ...
          {!aiThinking && !gameOver && currentStage % 2 === 1 && (
            <div style={s.moveSection}>
              ...
              <button style={s.stopBtn} onClick={() => handleMove("stop")}>
                <span style={s.btnTitle}>Stop</span>
                <span style={s.btnSub}>Take the payoff now</span>
              </button>
              <button style={s.continueBtn} onClick={() => handleMove("continue")}>
...
const s = {
  ...
  stageBadge: { fontSize: "13px", color: "#c0392b", fontWeight: "700", textTransform: "uppercase", letterSpacing: "1px" },
  backToMenu: {
  ...
  th: {
    ...
    backgroundColor: "#111", fontWeight: "700",
    fontSize: "12px", textTransform: "uppercase", letterSpacing: "1px",
  },
  td: { border: "1px solid #1f1f1f", padding: "11px 12px", color: "#888", textAlign: "center" },
  tdActive: {
    border: "1px solid #c0392b", padding: "11px 12px",
    color: "#ffffff", textAlign: "center",
    backgroundColor: "#1a0808", fontWeight: "700",
  },
  ...
  historyStage: { fontSize: "12px", color: "#c0392b", fontWeight: "700", width: "56px" },
  historyMover: { fontSize: "14px", color: "#888", flex: 1 },
  historyMove: { fontSize: "14px" },
  gameRight: {
  ...
  aiMoveText: { fontSize: "20px", color: "#ffffff", margin: 0, fontWeight: "700" },
  aiReasonText: { fontSize: "14px", color: "#666", margin: 0 },
  thinkingBox: {
  ...
  currentPayoff: {
    fontSize: "14px", color: "#777", margin: 0,
    padding: "12px 16px", backgroundColor: "#141414",
    borderRadius: "6px", border: "1px solid #1f1f1f",
  },
  stopBtn: {
  ...
  waitingBox: {
    ...
    borderRadius: "8px", border: "1px solid #1f1f1f", textAlign: "center",
  },
  waitingText: { fontSize: "16px", color: "#555", margin: 0, fontStyle: "italic" },
};
